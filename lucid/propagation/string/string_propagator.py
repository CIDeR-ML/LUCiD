...
def create_string_propagator(
    ...
    def propagate_fn(photon_origins, photon_directions):
        ...
        if vertical:
            ...
            w0 = o0[:, None] - ax0[None, :]
            w1 = o1[:, None] - ax1[None, :]
            origin_xy = jnp.sqrt(w0**2 + w1**2 + 1e-18)   # eps INSIDE sqrt: finite grad when a scattered photon lands on a string's xy axis (0/0 otherwise)
        ...
        ox, oy, oz = o0, o1, o2
        dx, dy, dz = d0, d1, d2
        # a_env floored at 1e-12 (not 1e-30) and the sqrt arg at 1e-18 so the
        # ALWAYS-computed barrel-t expression stays finite even for a (near-)vertical
        # scattered photon (dxy→0) or a tangent ray (disc→0). jnp.where only MASKS
        # the value; its reverse-mode gradient still flows through the unselected
        # branch, so an inf/NaN there would poison the whole optical-param gradient.
        a_env = dx**2 + dy**2 + 1e-12
        b_env = 2.0 * (ox * dx + oy * dy)
        c_env = ox**2 + oy**2 - env_r_sq
        disc_env = b_env**2 - 4 * a_env * c_env
        sqrt_disc = jnp.sqrt(jnp.maximum(disc_env, 1e-18))

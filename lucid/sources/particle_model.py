...
class ParticleModel(NamedTuple):
    """Everything needed to generate Cherenkov photon rays for a given
    particle type.

    Built via ``from_config(particle, material)`` which loads the Cherenkov
    SIREN model, its inference context, and the t0 timing parameters from
    the data directory.
    ...
    def from_config(particle: str = 'muon',
        ...
        """Load the Cherenkov SIREN model and parameters for a
        particle/material combination.
        ...
        context = build_cherenkov_context(predictor, siren_params['ray_sampling'])

...
* From the on-disk `{sensor,hits,step,labl}/wc_*.h5` tree — total bytes,
...
def measure_output_sizes(config_dir: Path) -> dict:
    ...
    Counts all `wc_*.h5` under `{sensor,hits,step,labl}/` plus the README.
    """
    streams = ["sensor", "hits", "step", "labl"]
...
def write_markdown(report: dict, out_md: Path) -> None:
    ...
    lines.append("")
    lines.append("| config | total H5 (MiB) | sensor (MiB) | hits (MiB) | step (MiB) | labl (MiB) | bytes / event |")
    lines.append("|---|---:|---:|---:|---:|---:|---:|")
    for cfg, data in report["configs"].items():
        if "error" in data:
            continue
        ...
        lines.append(
            ...
            f"{_fmt_n(per['step']['bytes'], 'MB-bytes')} | "

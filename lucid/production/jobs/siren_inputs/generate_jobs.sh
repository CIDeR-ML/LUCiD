#!/bin/bash
# generate_jobs.sh — cluster-agnostic shim around siren_inputs/generate_jobs.py.
#
...
#
# Usage: ./generate_jobs.sh -c <config_json> [-s] [-t] [-g]
...
# Walk up four levels (siren_inputs → jobs → production → lucid → LUCiD).
...
if [ "$NEEDS_USER_PATHS" -eq 0 ]; then
    if [ -f "${USER_PATHS}" ]; then
        ...
        "${APPTAINER_BIN:-apptainer}" exec \
            ...
            "${LUCID_IMAGE_PATH}" \
            /opt/conda/bin/python3 /opt/LUCiD/lucid/production/jobs/siren_inputs/generate_jobs.py -h
        exit 0
    fi

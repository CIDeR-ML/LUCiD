#!/bin/bash
# generate_jobs.sh — cluster-agnostic shim around train_siren/generate_jobs.py.
#
...
#
# Usage: ./generate_jobs.sh -c <scan_config_json> [-s] [-t]
#                                                 [--no-skip-existing]
...
# Walk up four levels (train_siren → jobs → production → lucid → LUCiD).
...
if [ "$NEEDS_USER_PATHS" -eq 0 ]; then
    if [ -f "${USER_PATHS}" ]; then
        ...
        "${APPTAINER_BIN:-apptainer}" exec \
            ...
            "${LUCID_IMAGE_PATH}" \
            /opt/conda/bin/python3 /opt/LUCiD/lucid/production/jobs/train_siren/generate_jobs.py -h
        exit 0
    fi
...
TMPOUT="$(mktemp -t lucid-train.XXXXXX)"
trap 'rm -f "${TMPOUT}"' EXIT

"${APPTAINER_BIN:-apptainer}" exec "${APPTAINER_OPTS[@]}" "${LUCID_IMAGE_PATH}" \
    /opt/conda/bin/python3 /opt/LUCiD/lucid/production/jobs/train_siren/generate_jobs.py \

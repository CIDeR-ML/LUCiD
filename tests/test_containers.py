...
class TestDetectorGeometry:
    def test_from_config_cylinder(self, small_cylinder_config):
        ...
        # 'cylinder', not the 'Cylinder' passed in: the field holds the value the config carries
        # (shipped configs spell it lowercase). The argument is only validated (see
        # test_invalid_detector_type); `generate_detector` reads the class from the JSON.

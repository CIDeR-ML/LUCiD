...
def test_scalar_qe_corrections_broadcasts_at_setup():
    ...
    assert dp.per_pmt.qe_corrections.ndim == 1, \
        f"Expected qe_corrections to be 1-D after setup, got ndim={dp.per_pmt.qe_corrections.ndim}"
    # All entries are the scalar placeholder value (1.0) by default.
    assert float(jnp.max(dp.per_pmt.qe_corrections)) == pytest.approx(1.0)
    assert float(jnp.min(dp.per_pmt.qe_corrections)) == pytest.approx(1.0)

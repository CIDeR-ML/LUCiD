"""Test the wall-intersection sqrt as the Hessian breaker: vary _CYL_SQRT_EPS (0 = legacy divergent
2nd-deriv) and measure the K=8 AD Hessian min-eig / cond / neg-diag + grad rel(AD,FD)."""
import os, sys
...
NPH = 40000; K = 8; MC = 4; GRID = dict(n_cap=80, n_angular=120, n_height=80)
...
print(f'NPH={NPH} K={K} MC={MC} ND={ND}\n_CYL_SQRT_EPS | min-eig    neg-diag    cond     grad rel(AD,FD)   |H|')
for eps in [0.0, 1e-6, 1e-4, 1e-2, 1e-1]:
    ...
    grel = np.linalg.norm(np.nan_to_num(g)-gF)/(np.linalg.norm(gF)+1e-9)
    print(f'  {eps:9.1e}  | {ev.min():10.2e}  {int((np.diag(H)<0).sum())}/9     {ev.max()/(abs(ev).min()+1e-30):8.1e}   {grel:6.3f}        {np.linalg.norm(H):.1e}')

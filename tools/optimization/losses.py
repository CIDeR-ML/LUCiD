...
#     w = jnp.where((true_q > threshold) & (true_q>1.), true_q, 0.)
...
#     threshold = jnp.percentile(true_q, 0.85)
#     w2 = jnp.where((true_q>0.) & (true_q>threshold), true_q, 0.)
...
#     #total_loss = jnp.sum(neg_loss_per_detector) / (jnp.sum(w) + 1e-8)
#     total_loss = jnp.sum(jnp.abs(pos_loss_per_detector*w)) / (jnp.sum(w) + 1e-8) / scale_w

#     return total_loss


# @jit
# def direction_time_loss(origin, direction, detector_positions, true_times, true_q, t0, photosensor_radius=0.25, c_medium=(0.299792/1.33), percentile_threshold=85., inv_scale_w=0.05, angle_scale_deg=3.0):

...
- **`t0` can be negative**. The writer jitters each event's emission time within roughly ±15 ns so the detector reads events at random clock phases (matches real-data triggering). Sensor/seg/inst times are stored as `t − t0` (detector frame, t=0 = trigger moment); the raw truth `t0` lives in `labl/per_event/t0`.
- **Units**. `sensor_positions` and all `seg/*` coordinates are in **meters**. Times are in **ns**. Energies in MeV.

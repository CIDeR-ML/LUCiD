#!/usr/bin/env python3
"""
TEST 5: Does K or Nphot cause the t0 bias?
Minimal: one data sim, two prediction sims (different K), 3 events.
"""
import sys, os
...
# Prediction K=9 (v2 default)
print("Building pred K=9...")
pred_K9 = setup_event_simulator(
    GEOM, Nphot, temperature=0.10, K=9,
    is_data=False, max_sensors_per_cell=4,
    physics_config=PHYS, default_detector_params=True)

# Prediction K=7 (clean-like)
print("Building pred K=7...")
pred_K7 = setup_event_simulator(
    GEOM, Nphot, temperature=0.10, K=7,
    is_data=False, max_sensors_per_cell=4,
    physics_config=PHYS, default_detector_params=True)

print("Running 3 events...\n")
...
for evt in range(3):
    ...
    for label, pred_sim in [("K=9", pred_K9), ("K=7", pred_K7)]:
        pred_q, pred_t = pred_sim(pp, key_p)

        losses = [float(cone_time_loss(data_q, pred_t, data_t, jnp.array(t), tau=0.23))
                  for t in t0_vals]
        cone_min = t0_vals[np.argmin(losses)]
        print(f"  evt {evt} {label}: cone_min@t0={cone_min:+.2f}")
    print()
